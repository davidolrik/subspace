...
Statistics are available via `subspace status`, the `/status` control socket endpoint, or the built-in statistics page at `stats.subspace.pub`. Historical data is persisted to a SQLite database with automatic downsampling; retention defaults to one year and is configurable via the [`stats`](/reference/configuration#stats) block.
